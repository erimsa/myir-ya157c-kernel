...
static int at9555_gpio_get_value(struct gpio_chip *chip, unsigned off)
{
	struct at9555_gpio *dev =
	    container_of(chip, struct at9555_gpio, gpio_chip);

	return !!(at9555_gpio_read(dev->client,
		  REG_INPUT_PORT0 + AT9555_BANK(off)) & AT9555_BIT(off));
}

static void at9555_gpio_set_value(struct gpio_chip *chip,
				   unsigned off, int val)
{
	...
	at9555_gpio_write(dev->client, REG_OUTPUT_PORT0 + bank,
			   dev->dat_out[bank]);
...
static int at9555_gpio_direction_input(struct gpio_chip *chip, unsigned off)
{
	...
	ret = at9555_gpio_write(dev->client, REG_CONFIGURATION_PORT0 + bank, dev->dir[bank]);
	mutex_unlock(&dev->lock);

	return ret;
}

static int at9555_gpio_direction_output(struct gpio_chip *chip,
					 unsigned off, int val)
{
	int ret;
	unsigned bank, bit;
					...
					const struct i2c_device_id *id)
{
	...
	printk(KERN_WARNING "i2c at9555 probe\n");
...
static const struct i2c_device_id at9555_gpio_id[] = {
	{"at9555", 0},
	{}
};
